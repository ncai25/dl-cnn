...
from convolution import conv2d as stu_conv2d
...
def sample_test():  # feel free to remove this
    ...
    stu_output = stu_conv2d(input_data, stu_filters, [1, 1, 1, 1], "SAME")
...
def padding_test_same():
    ...
    stu_output = stu_conv2d(
        input_data,
        np.ones(kernel_size + (input_data.shape[-1], filters)),
        (1,) + strides + (1,),
        "SAME",
    )
    # test shape
...
def weird_shapes_1_valid():
    ...
    # Use your custom conv2d function
    stu_output = stu_conv2d(input_data, stu_filters, [1, 1, 1, 1], "VALID")
